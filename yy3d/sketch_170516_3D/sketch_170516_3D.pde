...
nSpeed = new PVector(random(-.2, .2), random(-.2, .2), random(-.2, .2));//noise speed;
...
void mousePressed()
{
  ...
  nSpeed = new PVector(random(-.2, .2), random(-.2, .2));//reset noise speed
}
...
class Particle
{
  final float SPEED_MIN = .05, SPEED_MAX = .2, h = 40;
